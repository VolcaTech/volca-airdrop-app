...
On contract deployment special Ethereum account is generated by the airdropper's browser: address is stored to smart contract and the private key from the account is used to sign other claim private keys.  
Receiver gets a claim private key and signature (that the claim key is signed by the address stored to smart contract).  
Receiver signs his Ethereum address with claim private key and provides both signatures to Relayer Server. 
Relayer Server calls smart contract, which verifies both signatures and that the claim private key wasn't used before. 
If everything is correct, smart contract distributes tokens and ether to receiver's account. 
